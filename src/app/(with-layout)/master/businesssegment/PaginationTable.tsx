...
const PaginationTable = ({ setRefreshKey, onEdit, onAdd, onRowClick }: PaginationTableProps) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch(`/api/Marketing/BusinessSegment/get?t=${new Date().getTime()}`, { cache: 'no-store' });
                ...
                console.error("Error fetching Business Segment:", error);
                setData([]);
            } finally {
                setLoading(false);
            }
    ...
    const handleDelete = async (id: number) => {
        ...
        if (result.isConfirmed) {
            try {
                const response = await fetch(`/api/Marketing/BusinessSegment/delete/${id}`, {
                    method: "DELETE",
                });
                ...
                if (resultData && resultData.success) {
                    ...
                    Swal.fire(
                        "Deleted!",
                        resultData.message || "Business Segment has been deleted.",
                        "success"
                    );
                } else {
                    Swal.fire(
        ...
        () => [
            ...
            },
            {
                ...
                cell: (cell: any) => {
                    ...
                    const value = cell.getValue() || rowData.businessSegment || rowData.BusinessSegment;
                    return (
                        <span 
                    ...
                    return (
                        <div className="d-flex gap-2 justify-content-center">
                            ...
                            <button
                                ...
                                onClick={(e) => {
                                    e.stopPropagation();
                                    handleDelete(item.BusinessSegmentId || item.businessSegmentId);
                                }}
                            >
